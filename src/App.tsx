...
export default function App() {
  ...
  const executeDeepScan = async (urlToAnalyze: string) => {
    ...
    try {
      ...
      const [data, threatIntel] = await Promise.all([generateReportPromise, threatIntelPromise]);
          ...
          {report && !isAnalyzing && (
            ...
            >
              ...
              <div className="md:col-span-2 grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-6 items-start">
                ...
                {/* Jump to Navigation - Desktop */}
                <div className="sticky top-20 hidden lg:flex flex-col gap-1 bg-white border border-slate-200 rounded-md p-3 shadow-sm z-10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex lg:hidden overflow-x-auto gap-2 pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 hide-scrollbar sticky top-16 bg-slate-50 z-10 py-2 border-b sm:border-none border-slate-200">
                    <button onClick={() => scrollToSection('overview')} className={`shrink-0 text-left text-[10px] sm:text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${expandedSections.overview ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Summary</button>
                    <button onClick={() => scrollToSection('flags')} className={`shrink-0 text-left text-[10px] sm:text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${expandedSections.flags ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Indicators</button>
                    <button onClick={() => scrollToSection('recommendations')} className={`shrink-0 text-left text-[10px] sm:text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${expandedSections.recommendations ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Directives</button>
                    <button onClick={() => scrollToSection('relations')} className={`shrink-0 text-left text-[10px] sm:text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${expandedSections.relations ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Targets</button>
                    {report.timeline && report.timeline.length > 0 && (
                      <button onClick={() => scrollToSection('timeline')} className={`shrink-0 text-left text-[10px] sm:text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${expandedSections.timeline ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Timeline</button>
                    )}
                    {report.socialMediaPosts && report.socialMediaPosts.length > 0 && (
                      <button onClick={() => scrollToSection('social_media')} className={`shrink-0 text-left text-[10px] sm:text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${expandedSections.social_media ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Social</button>
                    )}
                  </div>

                  {/* Intelligence Summary */}
                  <Card id="overview" className="bg-white border border-slate-200 shadow-sm overflow-hidden scroll-m-28">
